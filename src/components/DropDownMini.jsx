...
const Dropdown = ({ options, onSelect, chevron, leftIcon, left, minWidth }) => {
  ...
  return (
    <div className="relative">
      <button
        className="px-3 py-2 rounded-lg shadow-lg bg-white text-grey text-xs font-medium flex items-center gap-2"
        onClick={handleToggle}>
        {selectedOption && (
            selectedOption.iconCode && <FlagIcon code={selectedOption?.iconCode} size={24} className='w-5 h-5  rounded-full' />
        )}
        { <HiOutlineFilter  className='w-5 md:w-7'/>}
        {selectedOption ? selectedOption.label : 'Filters'}
      </button>
      {isOpen && (
